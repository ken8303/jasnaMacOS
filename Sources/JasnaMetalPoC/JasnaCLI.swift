...
private func benchmark(runner: MetalDeformConv, weightSet: DeformConvWeightSet? = nil) throws {
    ...
    print("Gather+fused GEMM: \(String(format: "%.3f", result.simdgroupGEMMMedianMilliseconds)) ms")
    ...
    print("Gather-only median:       \(String(format: "%.3f", result.gatherStatistics.median)) ms")
    print("Fused matrix-only median: \(String(format: "%.3f", result.fusedMatrixStatistics.median)) ms")
...
func runJasnaCLI() async throws {
    ...
    try await commandLine.dispatch(.inspectSBSVideo) { inspectIndex in
    ...
    try await commandLine.dispatch(.transcodeSBS30) { transcodeIndex in
    ...
    try await commandLine.dispatch(.transcodeSBS30Tiled) { tiledIndex in
        guard commandLine.arguments.indices.contains(tiledIndex + 2) else {
            throw DeformConvError.commandFailed(
                "--transcode-sbs-30-tiled requires input and output .mov paths"
            )
        }
        let inputURL = URL(fileURLWithPath: commandLine.arguments[tiledIndex + 1])
        let outputURL = URL(fileURLWithPath: commandLine.arguments[tiledIndex + 2])
        let result = try await SideBySideVideoIO.transcodeTo30FPS(
            inputURL: inputURL,
            outputURL: outputURL,
            verifyTiledPixelPath: true
        )
        print("Tiled side-by-side 30 FPS video I/O: PASS")
        print("Chain:           decode → planar FP16 tiles → feather blend → HEVC")
        print("Input:           \(result.input.dimensions.width)×\(result.input.dimensions.height) at \(String(format: "%.3f", result.input.nominalFramesPerSecond)) FPS")
        print("Output:          \(result.output.dimensions.width)×\(result.output.dimensions.height) at \(String(format: "%.3f", result.output.nominalFramesPerSecond)) FPS")
        print("Frames written:  \(result.writtenFrameCount)")
        print("Output file:     \(outputURL.path)")
    }
    try await commandLine.dispatch(
    ...
    try await commandLine.dispatch(.restoreSBSEye) { restoreEyeIndex in
    ...
    try await commandLine.dispatch(.restoreEyeVideo) { restoreSingleEyeIndex in
    ...
    try await commandLine.dispatch(.restoreEyeWindows) { restoreWindowsIndex in
    ...
    try await commandLine.dispatch(.restoreEyeWindowsSparseBatch) { batchIndex in
    ...
    try await commandLine.dispatch(.diagnoseSBSTile) { diagnoseIndex in
    ...
    try await commandLine.dispatch(.benchmarkRealWeights) { realBenchmarkIndex in
    ...
    try await commandLine.dispatch(.metalMLBenchmark) { benchmarkIndex in
    ...
    try await commandLine.dispatch(.propagationSuite) { suiteIndex in
    ...
    try await commandLine.dispatch(.reconstructFrame) { frameIndex in
    ...
    try await commandLine.dispatch(.zeroCopyFrame) { graphIndex in
    ...
    try await commandLine.dispatch(.zeroCopyFrameGrouped) { graphIndex in
        guard commandLine.arguments.indices.contains(graphIndex + 2) else {
            throw DeformConvError.commandFailed(
    ...
    try await commandLine.dispatch(.zeroCopyFrameStaged) { graphIndex in
        guard commandLine.arguments.indices.contains(graphIndex + 2) else {
            throw DeformConvError.commandFailed(
    ...
    try await commandLine.dispatch(.zeroCopyFrameFused) { graphIndex in
        guard commandLine.arguments.indices.contains(graphIndex + 2) else {
            throw DeformConvError.commandFailed(
    ...
    try await commandLine.dispatch(.spynetPair) { spynetIndex in
    ...
    try await commandLine.dispatch(.frameWithSPyNet) { combinedIndex in
    ...
    try await commandLine.dispatch(.temporalInputs) { temporalIndex in
    ...
    try await commandLine.dispatch(.threeFrameRecurrence) { recurrenceIndex in
    ...
    try await commandLine.dispatch(.threeFrameFirstPass) { firstPassIndex in
    ...
    try await commandLine.dispatch(.threeFrameFourPass) { fourPassIndex in
    ...
    try await commandLine.dispatch(.variableClip) { variableIndex in
    ...
    try await commandLine.dispatch(.singleRunClip) { productionIndex in
    ...
    try await commandLine.dispatch(.validatePackageGraph) { graphIndex in
    ...
    try await commandLine.dispatch(.allocateFrameGraph) { arenaIndex in
        guard #available(macOS 27.0, *) else {
    ...
    try await commandLine.dispatch(.validateDeformWeights) { weightIndex in
